...
class Store(private var elems: ArrayBuffer[Value] = ArrayBuffer[Value]()) extends Value {
  ...
  override def toString = {
    var result = "{ "
    for(elem <- elems) result += (elem.toString() + " ")
    result += "}"
    result
  }
  ...
  def map(trans: Closure): Store = {
    new Store(elems.map((storeVal: Value) => trans.apply(List(storeVal))))
  }
  
  // returns store containing the elements of this that passed test
  def filter(test: Closure): Store = {
    new Store(elems.filter((storeVal: Value) => test.apply(List(storeVal)).asInstanceOf[Boole].value == true))
  }
}